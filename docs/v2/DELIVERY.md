...
## Power Ring 2.1: night mode, capture row, home button, snapshot mode (2026-09-26, overnight)

From the user's notes after trying V1:

- **Capture**: the main button opens the classic Windows Snipping Tool; behind it: selection copied (region snip), whole screen copied, and whole screen after a 3 s countdown (`"delay": 3` on `screen-to-clipboard`, 0-10 s). A delayed *region* selection is not offered by Windows' snip overlay, so the timer takes the whole screen, as the user accepted.
- **Night mode** (Manage): `close-apps` asks a predefined list of programs to close like clicking their X (they can still ask to save; never kills), and never closes Power Ring, Explorer or Claude (validation refuses them). `power-mode` switches the Windows power mode (efficiency / balanced / performance) through `PowerSetActiveOverlayScheme`; verified on this laptop by reading and re-applying the current overlay. MSI Center's own modes (Balanced / Extreme / AI / ECO) have no public interface, so the user's layouts add a button that opens MSI Center.
- **Home button**: on the first workspace, the ring's centre opens the clipboard board; the board and gallery footers show a larger home button (first workspace) between the previous/next workspaces, and the panel is placed so that the home button sits under the pointer: going back never needs a mouse move. Centre switchers slightly larger.
- **Layout**: circle-2 buttons keep a full spacing gap (the user's Work ring was too tight).
- **`--snapshot out.png --profile N`**: renders a workspace to a PNG without a window, hotkey or tray, for visual checks when the desktop is in use.
- The user's layouts (personal files) regenerated: V1 home lightened (VS Code, text editor, ChatGPT, Gmail, Explorer, capture, portfolio) with a new "Outils & news" workspace (Power Ops + DataGrip/PyCharm/Tabular Editor, formations, Edge perso/pro + Opera, BBC/CNN/NRK); circle 2 larger on V1 (37); the capture row and night mode applied to V1, V2 and V3.

Evidence: `build.ps1` PASS (smoke, 111/111, Power Ring **19/19**, with the new night-mode validation test). Offscreen snapshots of the six V1 workspaces checked. `tests/native/power-ring.ps1`: every behaviour check passed in the first run, but "idle: no CPU" read 281 ms / 5 s (then 94 ms) while the user was using the PC (clipboard activity reaches the listener of the test's clipboard table), and the second run was interrupted by focus changes; to re-run on an idle desktop.
